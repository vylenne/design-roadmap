  ...
  <div>
    <div class="flex flex-col justify-center items-center w-full lg:w-1/4">
      ...
      <h2 class="text-2xl font-semibold my-2 lg:my-4 text-center">
        {{ $t(`uiux.categories.${id}.title`) }}
      </h2>
    </div>
    <div v-for="(theme, i) in dataOfBlock" :key="i" class="border-b border-dashed last:border-0 px-4 lg:p-0">
      <PathLineSmall class="w-full lg:w-1/4 flex items-center mt-2 lg:mt-5" :class="i === 0 ? 'mt-0' : ''" />
      <div class="flex flex-col lg:flex-row items-center">
        ...
        <div
          ...
          <div v-for="(item, n) in theme.themes" :key="n"
            ...
            :class="n === 0 ? 'mt-0' : ''" @click="open(item)">
            {{ item }}
          </div>
        </div>
      </div>
